...
We use example forecast and target data bundled in the `modelimportance` package, originally from the `hubExamples` package that provides sample datasets for multiple modeling hubs in the hubverse format.
...
```{r fig-example-median-lomo, echo=FALSE}
target_data |>
...
Since we explored the difference of `na_action` options in the previous LOMO example above, we focus on options for `subset_wt`, which specifies how weights are assigned to subsets of models when calculating importance scores, with `na_action` fixed to `"drop"`.
