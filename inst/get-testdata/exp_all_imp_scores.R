...
for (k in seq_along(output_types)) {
  ...
  if (output_types[k] != "median") {
    ...
    all_df_score_results <- rbind(
      df_score_results_simple,
      df_score_results_lp
    )
    all_df_result_imputed <- rbind(
      df_result_imputed_simple,
      df_result_imputed_lp
    )
  } else {
  ...
  # save the result
  file_name1 <- paste0(
    "testdata/for-model_importance/exp_raw_imp_scores_",
    output_types[k],
    ".rds"
  )
  file_name2 <- paste0(
    "testdata/for-model_importance/exp_imputed_imp_scores_",
    output_types[k],
    ".rds"
  )
