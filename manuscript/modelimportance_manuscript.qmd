...
We highlight our quality assurance measures and its open access in \hyperref[sec:implementation-and-availability]{Section~\ref{sec:implementation-and-availability}}, and then 
...
`summary()` provides a concise summary of the importance scores, including key statistics such as number of models and tasks evaluated. 
...
- `.$task_winners` identifies which model is the best for each task based on the highest importance score across the full set of tasks. This information allows users to quickly identify the most important model in each task. 
...
input data, including the number of dates on which forecasts were produced, the number of models with ids, and whether the prediction tasks meet the minimum model requirement, alongside with a note on how to use parallel processing to speed up the computation when there are many models and tasks.
...
```
\normalsize
As indicated in the output, more details of the summary are accessed through the elements of the summary object as below.
```{r}
#| eval: false
...
Models with the highest importance scores for each task are identified in the `top_model` column with its importance score in the `max_score` column. 'PSI-DICE' is the best model for three out of the four tasks, while 'Flusight-baseline' is the best for the remaining task.
...
We conducted a computational experiment with simulated date. The execution time was measured for both algorithms across varying numbers of models and tasks. We also compared the execution times between sequential and multisession computing environments to demonstrate the efficiency of parallelization in handling computationally intensive tasks. 

All experiments were performed on a machine running macOS 15 with an Apple M4 chip and 16 GB RAM under R version 4.4.3.
...
@fig-feasibility-comparison illustrates the computational runtime for 4 different numbers of prediction tasks (10, 20, 50, and 100) and varying numbers of models (ranging from 2 to 10) for both the LOMO and LASOMO algorithms under sequential and parallel execution modes, indicated by color. The scale of the $y$-axis approximately range 0 to 12 seconds for LOMO and 0 to 1110 seconds for LASOMO.
...
This highlights the trade-off between the comprehensiveness of the LASOMO algorithm and its computational challenges in practice, particularly with large number of models.
