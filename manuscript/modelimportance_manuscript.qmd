...
Each ensemble excludes exactly one model while including all the others. Denoting by $F^{{\cal A}^{-i}}$ an ensemble forecast constructed without $F^i$ and by $F^{\cal A}$ the ensemble forecast built from the entire set of models, the importance score using LOMO is calculated as the difference in accuracy, as measured by a specific score, between $F^{{\cal A}^{-i}}$ and $F^{\cal A}$  (\hyperref[alg:lomo]{Algorithm~\ref{alg:lomo}}). 
...
In this example, there are only three models ($n=3$), and the weights do not differ significantly between the two methods. Therefore, the resulting outputs show little difference. However, in general, with a larger number of models, the two weighting schemes may yield different importance scores for each model.
