...
@kim2026beyond demonstrate that a model's individual performance does not necessarily correspond to its contribution as a component within an ensemble.
...
Inspired by these approaches, @lipiecki2024postprocessing analyzed the most contributing model to a combined ensemble with Shapley values on probabilistic prediction models converted from point predictions via their postprocessing methods, and as a follow-up, the [PostForecasts.jl]{.pkg} [@Jlpackage-lipiecki2025postforecast] package was developed to implement the methods. [PostForecasts.jl]{.pkg} shares some similarity with our approach in that it leverages Shapley values to assess component forecasters within the ensemble. 
...
We apply this idea to many tasks and aggregate the importance scores for that model across all tasks using averages. (Details can be found in @kim2026beyond.) 
...
scenarios with a larger number of models can be found in @kim2026beyond.
