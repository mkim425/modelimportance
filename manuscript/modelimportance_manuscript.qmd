...
data.frame(
  ...
  Description = c(
    "Root squared error (RSE): the root squared difference between the predicted value and the observed value",
...
The `importance_algorithm` argument specifies the algorithm for model importance calculation, which can be either `"lomo"` (leave one model out) or `"lasomo"` 
...
weight to all subsets and `"perm_based"` assigns weight averaged over all possible permutations, as in the formula of Shapley values (\hyperref[alg:lasomo]{Algorithm~\ref{alg:lasomo}}). The default values of `importance_algorithm` and `subset_wt` are `"lomo"` and `"equal"`, respectively.
...
`plot()` visualizes the importance scores as bar plots across models, with panels faceted by combinations of task ID values. Thus, users can compare importance scores across models and tasks. Additional modifications to the plot can be applied using standard `ggplot2` functions. 
...
An arithmetic mean (`fun = mean`) is the default, but other summary functions (e.g., `fun = median`) may also be used.
...
Our example forecast data contains short-term predictions of weekly incident influenza 
hospitalizations in the US for Massachusetts (FIPS code 25) and Texas (FIPS code
48), generated on November 19, 2022. 
These forecasts are made for two target end dates, November 26, 2022 (horizon 1), and
...
We visualize the importance scores across models and tasks using the `plot()` method, which allows for further modification with `ggplot2` functions. Since `plot()` flips the axes by default, we reformatted the model IDs (originally appearing on the $x$-axis) via `scale_x_discrete()`, which replaces hyphens and underscores with line breaks to save horizontal space and improve readability. 
We further applied `theme()` to rotate the $x$-axis labels and adjust panel spacing, preventing label overlap. 
...
\vspace{0.35cm} 
\normalsize
The results show that, overall, the model 'PSI-DICE' has the highest importance score, 
...
This package is implemented in [R]{.proglang} and distributed via CRAN and GitHub, under the MIT license (\url{https://github.com/mkim425/modelimportance}). 
