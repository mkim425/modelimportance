...
#| tbl-cap: "Description of the arguments for the `model_importance_summary()` function, including their purpose, possible values, and default settings."
...
`model_importance_summary()` function returns an object of S3 class `importance_summary`, with columns 
...
```{r code-model_importance-summary-lomo-drop}
#| eval: false
> scores_lomo <- model_importance(
        forecast_data = forecast_data,
        oracle_output_data = target_data,
...
```{r code-model_importance-summary-lomo-worst}
#| eval: false
> model_importance_summary(
        importance_scores = scores_lomo, 
        by = "model_id", 
...
```{r code-model_importance-summary-lomo-avg}
#| eval: false
> model_importance_summary(
        importance_scores = scores_lomo, 
        by = "model_id", 
...
## Classes and S3 methods {#sec:s3-infrastructure}

We have defined S3 classes and methods to facilitate the use of the main functions in the package. In this section, we describe 
the S3 infrastructure underlying our package, including `print`, `summary`, and `plot` methods for the output of the main functions.

### model_importance_tbl class
