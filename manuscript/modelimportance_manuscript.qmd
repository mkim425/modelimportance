...
\label{alg:lomo}
  \begin{spacing}{1.3}
	...
	$\{F^1,F^2,...,F^n\}$  
        ...
        \Statex $\bullet$ scoring rule $\psi$ to evaluate
        forecast skills 
        }
     \ENSURE importance metric of model $i$,
     $\phi^{i,\text{lomo}}$
     \STATE{
     create an ensemble forecast $F^{\cal A}$ using $g$:
     $F^{\cal A} \leftarrow g(\{F^1,F^2,...,F^n\})$}
