...
We conducted a computational experiment using simulated data with a point forecast ('mean'). 
...
The speedup from parallel execution relative to sequential execution is approximately 4-fold for LOMO in these settings of 10, 20, 50, and 100 tasks, which is consistent with the use of four worker processes. 
...
Theoretically, the computational complexity of LOMO and LASOMO algorithms is $O(t \cdot n)$ and $O(t\cdot 2^n)$, respectively, where $n$ is the number of models and $t$ is the number of tasks. 
However, the observations from our computational experiment in the sequential execution mode, where the number of models ranges from 2 to 10 and the number of tasks ranges from 10 to 100,
show slight discrepancies from these theoretical complexities.
While the observed runtimes show linear growth with respect to the number of tasks for both algorithms, consistent with the theoretical complexities, the growth with respect to the number of models is sublinear for LOMO and slower than the expected exponential for LASOMO. 
...
We note that the scaling increment and ratio between adjacent values of $n$ in LOMO and LASOMO, respectively, is gradually increasing, which implies the empirical complexity is approaching the theoretical complexity as the number of models increases. 
...
<!--  This phenomenon is also observed even in the parallel execution setting (@fig-feasibility-parallel(B)).--->    
