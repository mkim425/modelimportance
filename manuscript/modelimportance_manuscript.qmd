...
forecast_data <- hubExamples::forecast_outputs |>
  ...
  mutate(
    model_id = dplyr::recode(model_id,
      "Flusight-baseline" = "model A",
      "MOBS-GLEAM_FLUH" = "model B",
      "PSI-DICE" = "model C"
    ),
    value = ifelse(value > 100, value / 50, value) |> round()
  ) |>
  filter(
    !(model_id == "model B" & location == "25" & target_end_date == as.Date("2022-11-26")),
    !(model_id == "model C" & location == "48" & target_end_date == as.Date("2022-12-10"))
  )
...
\normalsize
### Evaluation using untrained ensemble in LASOMO algorithm for quantile-based forecasts  {#sec:untrained-lasomo-qntl}
