...
tbl_argument <- data.frame(
  "Argument" = c(
    ...
    "\\texttt{ensemble\\_fun}", "\\texttt{weighted}",
    "\\texttt{training\\_window\\_length}",
  ...
  "Description" = c(
    ...
    "Ground truth data",
    "Ensemble method",
    "Whether to use model weighting when building an ensemble",
...
The `weighted` argument is a logical value that indicates whether model weighting should be done when building an ensemble using the specified `ensemble_fun`. The value of `FALSE` means that all component models are equally weighted in the ensemble construction.
If it is set to `TRUE`, model weights are estimated based on the previous performance of each model using historical data within a training window defined by the `training_window_length` argument. The non-negative value of `training_window_length` specifies the number of most recent time units (e.g., days or weeks) to consider when evaluating past model performance. Its default value is `0`, indicating no training for model weighting, thus it is equivalent to setting `weighted = FALSE`.
...
### Evaluation using untrained ensemble in LASOMO algorithm for mean forecasts  {#sec:untrained-lasomo-mean}
