...
abstract: |
  Ensemble forecasts are commonly used to support decision-making and policy planning across various fields because they often offer improved accuracy and stability compared to individual models. While a range of ensemble methods exist, understanding how much each constituent model contributes to the resulting ensemble performance is a separate but important consideration. The [R]{.proglang} package [modelimportance]{.pkg} addresses this need by providing a set of tools to quantify the importance of each component model within an ensemble. This functionality complements existing resources in the ['hubverse'](https://hubverse.io/) framework $-$ a collection of open-source software, tools and data standards developed to promote collaborative modeling hub efforts and simplify their setup and operation $-$ by extending the capabilities of [hubEnsembles]{.pkg} for ensemble construction and [hubEvals]{.pkg} for ensemble evaluation.
  Our package supports multiple ensemble methods and multiple model importance metrics for both point and probabilistic forecasts. Additionally, the software offers customizable options for handling missing values. These features enable the package to serve as a versatile tool for researchers and practitioners. It helps not only in constructing an effective ensemble model across a wide range of forecasting tasks, but also in understanding the role of each model within the ensemble and gaining insights into individual models themselves. Following the ['hubverse'](https://hubverse.io/) standards enables seamless integration and flexibility with other forecasting tools and systems, allowing many analyses to be performed on existing hubs.
...
A variety of ensemble construction methods have been proposed, ranging from simple averaging to more sophisticated approaches based on performance-weighted averaging, Bayesian model averaging, stacking, linear opinion pools, and machine learning [@wang2023forecast; @reich_accuracy_2019; @ranjan2010combining]. 
In the 'hubverse' framework, a community-maintained ecosystem of open software and data standards for collaborative forecasting hubs [@hubverse_docs; @kerr2025coordinating], the [hubEnsembles]{.pkg} (@Rpackage-hubEnsembles) provides a standardized approach to constructing ensembles via simple averaging and linear pooling, which the [modelimportance]{.pkg} builds upon rather than duplicating. 
Regardless of the ensemble construction method, a separate but important consideration is to understand how much each constituent model contributes to the resulting ensemble performance, and this lies at the core of our package. 
These insights will help to identify the most influential models, which can inform model selection, weighting, and refinement strategies (@fig-workflow_diagram).

```{r, fig.pos="t"}
#| echo: false
#| label: fig-workflow_diagram
#| fig-cap: "Example workflow illustrating how [modelimportance]{.pkg} fits within the hubverse ensemble forecasting pipeline. Component model forecasts are combined into an ensemble using [hubEnsembles]{.pkg}, and the resulting ensemble is scored using [hubEvals]{.pkg}. [modelimportance]{.pkg} then quantifies each component model's contribution to ensemble performance. These importance scores can, in turn, inform the selection or re-weighting of component models when constructing a revised ensemble (dashed feedback arrow), closing the loop between model evaluation and ensemble construction."
#| out.width: "100%"
#| fig-align: "center"
knitr::include_graphics("figure-workflow_diagram.pdf")
```
...
The [scoringutils]{.pkg} [@bosse2022evaluating] package offers additional features to the functionality provided by [scoringRules]{.pkg}, which makes it more useful for certain tasks, such as summarizing, comparing, and visualizing forecast performance. 
...
In ensemble forecasting, certain models contribute more significantly to the overall predictions than others. Assessing the impact of each component model on ensemble predictions is methodologically similar to determining variable importance in traditional regression and machine learning models, where variable importance measures evaluate how much individual variables improve the accuracy of the model's predictive performance or reduce the average loss. [R]{.proglang} packages such as [randomForest]{.pkg} [@Rpackage-randomForest], [caret]{.pkg} [@Rpackage-caret], [xgboost]{.pkg} [@Rpackage-xgboost], and [gbm]{.pkg} [@Rpackage-gbm] implement these functions for different types of models: random forest models, general machine learning models, extreme gradient boosting models, and generalized boosted regression models, respectively.
...
By quantifying each component model's contribution, [modelimportance]{.pkg} can support evidence-based decisions about ensemble design and maintenance. The package follows conventions defined by the hubverse. Using these shared data structures allows our package to be seamlessly integrated with existing hub pipelines and deployed across multiple active hubs. We note that there are 31 hubs as of March 2026, including model development and training hubs.

<!-- 
A variety of ensemble construction methods have been proposed, ranging from simple averaging to more sophisticated approaches based on performance-weighted averaging, Bayesian model averaging, stacking, linear opinion pools, and machine learning [@wang2023forecast; @reich_accuracy_2019; @ranjan2010combining]. While a number of these methods allow to combine predictive models to achieve a better overall predictive performance, empirical work in the domain of infectious disease forecasting has found that fancy ensemble methods do not usually show dramatic improvements over simple averaging [@ray_prediction_2018]. Given this, the [hubEnsembles]{.pkg} (@Rpackage-hubEnsembles), a package within the hubverse infrastructure, implements simple averaging and linear pooling to construct an ensemble forecast from multiple component models, which [modelimportance]{.pkg} builds upon rather than duplicating. 
--> 
...
\hyperref[sec:implementation-and-availability]{Section~\ref{sec:implementation-and-availability}} highlights our quality assurance measures and the code's availability, and then we close this paper with a summary and a discussion of possible extensions.
...
The [modelimportance]{.pkg} package is designed to work with the hubverse framework and, accordingly, depends on several packages in the hubverse ecosystem, such as [hubUtils]{.pkg} (@Rpackage-hubUtils), [hubEnsembles]{.pkg} (@Rpackage-hubEnsembles), and [hubEvals]{.pkg} (@Rpackage-hubEvals).
...
#| hospitalizations (top 10 rows) extracted from `forecast_data_example` bundled in the
...
In this section, we illustrate how to use the `model_importance()` function to evaluate the importance of component models within an ensemble. The examples show various combinations of the arguments described in \hyperref[sec:main-function]{Section~\ref{sec:main-function}}. 
Example forecast and target data are originally sourced from the [hubExamples]{.pkg} package, which provides sample datasets for multiple modeling hubs in the hubverse format. 
...
The [modelimportance]{.pkg} package is implemented in [R]{.proglang} and distributed via CRAN and GitHub, under the MIT license (\url{https://github.com/mkim425/modelimportance}). 
We conducted unit tests using the [testthat]{.pkg} package [@Rpackage-testthat] to verify that all inputs and outputs are properly formatted and ensure that all functions work correctly as expected, including those used internally. 
