...
This section describes computational complexity of LOMO and LASOMO algorithms implemented in the package, depending the numbers of models and tasks.
...
@fig-feasibility-comparison illustrates the efficiency of parallel computation in both algorithms. With the tasks of 10 or 20, the difference between the computation times of sequential and parallel execution is not significant. However, as the prediction tasks increase to 50, the difference becomes more pronounced. 
Considering the different scales between the two algorithms, the efficiency of parallel execution is more pronounced for LASOMO than for LOMO. 


```{r, fig.pos="t"}
#| echo: false
#| label: fig-feasibility-comparison
#| fig-cap: "Elapsed time by number of models and tasks (Sequential vs Parallel execution). Color denotes the execution mode (*coral* for sequentail and *steelblue* for parallel)."
#| out.width: "100%"
#| fig-align: "center"
...
@fig-feasibility-parallel displays the computational runtime under the parallel execution mode.
In LOMO, the elapsed time increases linearly with the number of models, and the rate of increase depends on the number of tasks, with a steeper increase as the number of tasks grows. 
In contrast, in LASOMO, the elapsed time increases much more rapidly as the ensemble size grows. In particular, when the number of models exceeds 12, it takes more than 20 minutes to compute the importance scores, even with fixed 5 tasks. 
It is because the number of subsets of models that need to be evaluated in LASOMO grows exponentially with the number of models, leading to a combinatorial explosion in the computational requirements.
This highlights the computational challenges of LASOMO as the number of models grows.
...
#| fig-cap: "Elapsed time by number of models and tasks during parallel execution via multisession backend."
#| out.width: "100%"
#| fig-align: "center"
