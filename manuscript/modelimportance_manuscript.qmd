...
The `model_importance()` function returns an object of S3 class `model_imp_tbl`, with columns 
...
`model_id` and `importance_score_<fun>`, where `<fun>` is the name of the summary function used (e.g., `importance_score_mean` when `fun = mean`). The output is sorted in descending order of the summarized importance scores, `importance_score_<fun>`.
