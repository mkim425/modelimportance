...
Parallel computations were implemented using the [future]{.pkg} (`multisession` backend) with 4 worker processes. 
...
@fig-feasibility-comparison illustrates the computational runtime for 4 different numbers of prediction tasks (10, 20, 50, and 100) and varying numbers of models (ranging from 2 to 10) for both the LOMO and LASOMO algorithms under sequential and parallel execution modes, indicated by color. The scale of the $y$-axis approximately ranges from 0 to 12 seconds for LOMO and from 0 to 1110 seconds for LASOMO.
...
This highlights the trade-off between the comprehensiveness of the LASOMO algorithm and its computational challenges in practice, particularly with a large number of models.

## Implementation and availability {#sec:implementation-and-availability}

This package is implemented in [R]{.proglang} and distributed via CRAN and GitHhub (\url{https://github.com/mkim425/modelimportance}). 
