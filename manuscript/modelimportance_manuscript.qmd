...
`plot()` visualizes the importance scores across models faceted by task IDs. Thus, users can compare importance scores across models and tasks.
...
We visualize the importance scores across models and tasks using the `plot()` method. 
```{r}
#| eval: false
> plot(scores_lomo)
```
