...
We note that CRPS is a general scoring rule that can be computed either analytically in closed form or numerically from samples, and WIS is a quantile-based approximation of CRPS.
...
We describes the S3 class structure and related methods implemented in the package in  \hyperref[sec:s3-infrastructure]{Section~\ref{sec:s3-infrastructure}}, followed by examples of their usage in \hyperref[sec:examples]{Section~\ref{sec:examples}}.
...
We note that the scoring rules are listed in the table using their conventional names, however, the package uses their negative values for evaluation (e.g., $-\text{RSE}, -\text{WIS}$) so that higher scores indicate better performance. 
This positively oriented scoring rule facilitates the interpretation of importance scores: positive values indicate that the model's inclusion in the ensemble improves ensemble performance, whereas negative values indicate that it worsens it. Also, a higher importance score indicates a greater contribution of the model to the ensemble's predictive performance, which aligns with the convention of Shapley values.
...
all component models per prediction task defined by the task IDs, `output_type`, and `output_type_id` columns. 
