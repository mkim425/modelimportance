...
We conducted unit tests using the [testthat]{.pkg} package [@Rpackage-testthat] to ensure the correctness of all functions, including those used internally. Additionally, we performed continuous integration testing using GitHub Actions to maintain code quality and functionality across platforms, including Windows, macOS, and Linux. 
...
forecast_data |>
  ...
  geom_point(
    data = target_data,
    aes(y = oracle_value, group = 1, color = "Observed"),
    shape = 1, alpha = 1, size = 2
  ) +
  scale_x_date(breaks = target_data$target_end_date, 
               date_labels = "%Y-%m-%d",
               expand = expansion(add = c(5, 5)) )+
  scale_color_manual(name = "model_id/Observed",
                     values = c("Flusight-baseline" = "#619CFF", 
                                "MOBS-GLEAM_FLUH" = "#00BA38", 
                                "PSI-DICE" = "#F8766D", 
                                "Observed" = "black"),
                     limits = c("Flusight-baseline", 
                                "MOBS-GLEAM_FLUH", 
                                "PSI-DICE", 
                                "Observed")) +
  labs(
...
The following code evaluates the importance of each model in the simple mean ensemble using the LOMO algorithm, without training the ensemble (`weighted = FALSE`); training is infeasible as the data available for training is limited. The `na_action` argument is set to `"drop"`, which represents that any missing values in the forecasts will be excluded from the evaluation. 
