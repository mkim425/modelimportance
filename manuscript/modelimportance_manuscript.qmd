...
This call generates informative messages, summarizing the 
input data, including the number of dates on which forecasts were produced, the number of models with ids, and whether the prediction tasks meet the minimum model requirement, alongside a note on how to use parallel processing to speed up the computation when there are many models and tasks.
...
Calling `summary()` shows that three models were used and four tasks were evaluated, along with a sneak peek of the top-performing model for each task.
```{r}
#| eval: false
> summary(scores_lomo)
```
...
As we explored the difference of `na_action` options in the previous
...
```{r code-model_importance-summary-lasomo-equal, eval=FALSE}
#| eval: false
...
```{r code-model_importance-summary-lasomo-perm, eval=FALSE}
#| eval: false
