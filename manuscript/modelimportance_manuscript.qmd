...
In general, the differences in how the two weighting schemes influence the importance scores become more pronounced as the number of models increases.
Recall that the formulas for their subset weights are 
$$\frac{1}{2^{n-1}-1} \quad\text{and}\quad \frac{1}{(n-1)\binom{n-1}{k}},$$ 
where $k$ is the size of each subset and $n$ is the total number of models, as described in \hyperref[alg:lasomo]{Algorithm~\ref{alg:lasomo}}.
...
This describes the mathematical behavior of the two weighting schemes, however, 
in practice, the difference between the two weighting schemes may arise primarily from the the extreme-sized subsets when $n$ is large. 
...
```{r plot-lasomo-weights, fig.pos="t"}
#| echo: false
