...
The `oracle_output_data` is a data frame containing the actual observed values of the variables used to specify modeling targets. Details are provided in Section \ref{subsec:oracle_output_data}.
...
The subset weights are assigned equally or based on their sizes as described in Algorithm \ref{alg:lasomo}. Recall the formulas for these weights:
...
In this example, there are only three models ($n=3$), and the weights do not differ significantly between the two methods. Therefore, the resulting outputs do not show large differences. However, in general, with a large number of models, the two weighting schemes may produce different importance scores for each model.
More specifically, while the weights under the equal weighting scheme decreases exponentially with $n$, the  permutation-based weights decreases polynomially.
In addition, the equal scheme treats all subsets equally, so the influence of medium-sized subsets is large in the final result as there are many such subsets. In contrast, the permutation-based scheme adjusts the weights according to the different sizes of the subsets, giving the largest weight to the smaller and larger subsets. Therefore, when $n$ is large, middle-sized subsets dominate in equal weighting scheme while extreme sized subsets dominate in permutation-based weighting scheme.
