...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, boxrule=0pt, enhanced, borderline west={3pt}{0pt}{shadecolor}, interior hidden, frame hidden, breakable]}{\end{tcolorbox}}\fi
\makeatother
\usepackage{bookmark}
...
with some concluding remarks and a discussion of possible extensions.
...
\addlinespace
\texttt{na\_action} & Method to handle missing data & \texttt{"worst", "average", "drop"} & \texttt{"worst"}\\
\cellcolor{gray!10}{\texttt{min\_log\_score}} & \cellcolor{gray!10}{Minimum value to replace for log score} & \cellcolor{gray!10}{Non-positive numeric} & \cellcolor{gray!10}{-10}\\
...
algorithm. The \texttt{na\_action} argument is set to \texttt{"drop"},
which represents that any missing values in the forecasts will be
...
In general, the differences in how the two weighting schemes influence
the importance scores become more pronounced as the number of models
increases. Recall that the formulas for their subset weights are
...
\hyperref[alg:lasomo]{Algorithm~\ref{alg:lasomo}}. The equal scheme
treats all subsets equally, so medium-sized subsets have considerable
influence in the final result, as there are many such subsets. In
contrast, the permutation-based scheme adjusts the weights according to
the subset size, giving the greatest weight to both the smallest and
largest subsets while assigning small weights to the mid-sized subsets.
Moreover, the weights assigned to the mid-sized subsets under the
permutation-based approach decrease much faster with \(n\) than those
under the equal weighting scheme (see
...
the permutation-based weighting approach. This describes the
mathematical behavior of the two weighting schemes, however, in
practice, the difference between the two weighting schemes may arise
primarily from the the extreme-sized subsets when \(n\) is large. This
