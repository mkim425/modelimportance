...
predictive accuracy when used as part of an ensemble.
\citet{kim2026beyond} demonstrate that a model's individual performance
does not necessarily correspond to its contribution as a component
within an ensemble. Our developed package introduces this capability.
The \pkg{modelimportance} package provides tools to evaluate the role of
...
\citet{lundberg2020local}; \citet{covert2020understanding}). Inspired by
these approaches, \citet{lipiecki2024postprocessing} analyzed the most
contributing model to a combined ensemble with Shapley values on
probabilistic prediction models converted from point predictions via
their postprocessing methods, and as a follow-up, the
\pkg{PostForecasts.jl} \citep{Jlpackage-lipiecki2025postforecast}
package was developed to implement the methods. \pkg{PostForecasts.jl}
shares some similarity with our approach in that it leverages Shapley
values to assess component forecasters within the ensemble. However,
while \pkg{PostForecasts.jl} was developed for \proglang{Julia} users
...
Our various capabilities provide unique support for hub organizers who
...
\hyperref[sec:main-function]{Section~\ref{sec:main-function}} and
highlight our quality assurance measures and its open access in
...
\hyperref[sec:examples]{Section~\ref{sec:examples}}. We close this paper
with concluding remarks and a discussion of possible extensions.
...
apply this idea to many tasks and aggregate the importance scores for
that model across all tasks using averages. (Details can be found in
\citet{kim2026beyond}.)
...
computational challenges, particularly with large number of models.
