...
(MSE), which calculate the average magnitude of forecast errors. Proper
scoring rules for probabilistic forecasts consider the uncertainty and
...
CDC in the US and the European Centre for Disease Prevention and Control
in the EU. The \pkg{modelimportance} package can even strengthen the
benefits of a multi-model ensemble by helping these organizations create
more effective ensemble forecasts based on the precise evaluation of
each model's contribution. Specifically, \pkg{modelimportance} follows
...
formats specified by the hubverse convention enables seamless
integration and flexibility with other forecasting tools and systems.

The paper proceeds as follows. In Section \ref{sec:data}, we address the
model output formats proposed within the hubverse framework and the
structure of forecasts. Section \ref{sec:algorithms} presents two
...
\ref{sec:main-function} and give some examples in Section
\ref{sec:examples}. We close this paper with some concluding remarks and
...
ecosystem, such as \pkg{hubUtils}, \pkg{hubEnsembles}, \pkg{hubEvals},
and \pkg{hubExamples}. The \pkg{modelimportance} uses a
\texttt{model\_out\_tbl} S3 class as the model output format defined in
the \pkg{hubUtils}, which consists of utility functions to standardize
prediction files and data formats (details in Section
\ref{subsec:model_output_format}). Ensembling predictions from multiple
models relies on the \pkg{hubEnsembles}. Calculation of forecast
accuracy using various metrics is based on the \pkg{hubEvals}, which
internally leverage the \pkg{scoringutils}. We use or modify the example
datasets from the \pkg{hubExamples} for testing and demonstration
purposes (see Section \ref{sec:examples}).
...
an individual prediction for a single task, and its details are
described in multiple columns through which one can identify the model
IDs, task characteristics, prediction representation type, and predicted
values \citep{Shandross2024}. To elaborate on the task characteristics,
each prediction task means a specific forecasting problem and it is
defined by a set of task ID variables, which includes a date on which
...
target date. Table~\ref{tbl-example-model_output} illustrates short-term
forecasts of weekly incident influenza hospitalizations in the US for
Massachusetts, generated by the model `Flusight-baseline' on November
19, 2022, in the \texttt{model\_out\_tbl} format. The \texttt{model\_id}
column lists a uniquely identified model name. All of the
\texttt{reference\_date}, \texttt{target}, \texttt{horizon},
...
date for prediction are mapped to the \texttt{reference\_date} and
\texttt{target\_end\_date} columns, respectively, and the location is
represented based on the FIPS code (`25' for Massachusetts). The time
length to the \texttt{target\_end\_date}, the number of weeks ahead from
the \texttt{reference\_date}, is indicated in the \texttt{horizon}
...
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 1 & 25 & 2022-11-26 & quantile & 0.5 & 51\\
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 1 & 25 & 2022-11-26 & quantile & 0.75 & 64\\
\addlinespace
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 1 & 25 & 2022-11-26 & quantile & 0.9 & 81\\
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 1 & 25 & 2022-11-26 & quantile & 0.95 & 97\\
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 2 & 25 & 2022-12-03 & quantile & 0.05 & 0\\
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 2 & 25 & 2022-12-03 & quantile & 0.1 & 12\\
Flusight-baseline & 2022-11-19 & wk inc flu hosp & 2 & 25 & 2022-12-03 & quantile & 0.25 & 33\\
\bottomrule
\end{tabular}}
...
the target end dates of November 26, 2022 (horizon 1), December 03, 2022
(horizon 2), and December 10, 2022 (horizon 3), which were made on
November 19, 2022 based on the historical data available as of that
...
black circles indicate the eventually observed values. The blut dots
...
They are represented in various ways, such as probability mass functions
(pmf), cumulative distribution functions (cdf), samples, or probability
quantiles (or intervals).
...
``0.5'', ``0.75'', ``0.9'') for `quantile' and categorical values (e.g.,
``low'', ``moderate'', ``high'', and ``very high'') for `pmf'. The
predicted values for \texttt{pmf} are constrained to be between 0 and 1,
...
This data must follow the oracle output format defined in the hubverse
standard, which includes independent task ID columns (e.g.,
\texttt{location}, \texttt{target\_date}, and \texttt{age\_group}), the
...
column is required to provide further identifying information.
