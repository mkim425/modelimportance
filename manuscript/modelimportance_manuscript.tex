...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, enhanced, sharp corners, boxrule=0pt, interior hidden, frame hidden, breakable]}{\end{tcolorbox}}\fi
\makeatother
\usepackage{bookmark}
...
predictions. The ensemble model served as the official short-term
...
distribution functions \citep{bracher_evaluating_2021}. We note that
CRPS is a general scoring rule that can be computed either analytically
in closed form or numerically from samples, and WIS is a quantile-based
approximation of CRPS.
...
many teams into a single ensemble forecast \citep{Shandross2024}.
...
\citep{hubverse_docs}. Using these shared data structures allows our
package to be seamlessly integrated with existing hub pipelines and
deployed across multiple active hubs. We note that there are over a
dozen active hubs running as of fall 2025, with more in planning stages.
...
different operating systems, and independent code review by peer
developers. This emphasis on quality control is a key strength of this
work and distinguishes it from other academic software development
projects.

The paper proceeds as follows.
\hyperref[sec:data]{Section~\ref{sec:data}} describes how the
...
It is referred to as ``oracle'' because it is formatted as if an oracle
...
includes independent task ID columns (e.g., \texttt{location},
\texttt{target\_date}), the \texttt{output\_type} column specifying the
output type of the predictions and an \texttt{oracle\_value} column for
the observed values. As in the forecast data, if the
\texttt{output\_type} is either \texttt{"quantile"} or \texttt{"pmf"},
the \texttt{output\_type\_id} column is often required to provide
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figure-concept.png}
...
forecasting model with component model(s) shown inside. Vertical bars
indicate the performance score measured by a positively oriented scoring
rule (e.g., \(-\)WIS), with more bars indicating better accuracy. LOMO
computes a single performance difference by removing the target model A
from the full ensemble. LASOMO leverages multiple performance
differences across all subsets containing the model A and aggregates
them.}
...
including all the others. \(F^{{\cal A}^{-i}}\) denotes the ensemble
forecast constructed using all forecasts \(F^{\cal A}\) except \(F^i\).
Model \(i\)'s importance score using LOMO is calculated as the
difference in accuracy, as measured by a specific scoring rule, between
\(F^{{\cal A}^{-i}}\) and \(F^{\cal A}\)
...
the model \(i\), \(S \cup \{i\}\) plays the role of \({\cal A}\) in the
...
GitHhub (\url{https://github.com/mkim425/modelimportance}). We conducted
unit tests using the \pkg{testthat} package \citep{Rpackage-testthat} to
...
\pkg{lintr} package to maintain code quality and detect potential
issues, and the Air formatter to ensure consistent code style across the
codebase. All code changes were systematically reviewed by fellow team
...
metrics. Users can choose the various ensemble methods to apply and the
