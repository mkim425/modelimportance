...
\texttt{weighted} & Whether to use model weighting when building an ensemble & Logical: \texttt{TRUE}, \texttt{FALSE} & \texttt{FALSE}\\
\cellcolor{gray!10}{\texttt{training\_window\_length}} & \cellcolor{gray!10}{Time interval of historical data used during the training process} & \cellcolor{gray!10}{Non-negative integer} & \cellcolor{gray!10}{0}\\
\addlinespace
...
specified \texttt{ensemble\_fun}. The value of \texttt{FALSE} means that
all component models are equally weighted in the ensemble construction.
If it is set to \texttt{TRUE}, model weights are estimated based on the
previous performance of each model using historical data within a
training window defined by the \texttt{training\_window\_length}
argument. The non-negative value of \texttt{training\_window\_length}
specifies the number of most recent time units (e.g., days or weeks) to
consider when evaluating past model performance. Its default value is
\texttt{0}, indicating no training for model weighting, thus it is
equivalent to setting \texttt{weighted\ =\ FALSE}.
