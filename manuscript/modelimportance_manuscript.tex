...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, enhanced, interior hidden, boxrule=0pt, sharp corners, borderline west={3pt}{0pt}{shadecolor}, breakable]}{\end{tcolorbox}}\fi
\makeatother
\usepackage{bookmark}
...
\hyperref[sec:main-functions]{Section~\ref{sec:main-functions}} and
...
\section{Main functions}\label{sec:main-functions}
...
\subsection{model\_importance\_summary(
)}\label{model_importance_summary}
...
\texttt{model\_importance\_summary()} function, including their purpose,
possible values, and default settings.}

\end{table}%

The \texttt{model\_importance\_summary()} function summarizes the
importance scores produced by \texttt{model\_importance()} across tasks
for each model.
...
The \texttt{importance\_scores} is a data frame containing model
importance scores for individual prediction tasks, as produced by the
\texttt{model\_importance()} function.
...
\texttt{model\_importance\_summary()} function returns an object of S3
class \texttt{importance\_summary}, with columns \texttt{model\_id} and
...
\hyperref[sec:main-functions]{Section~\ref{sec:main-functions}}. We use
...
\section{Classes and S3 methods}\label{sec:s3-infrastructure}

We have defined S3 classes and methods to facilitate the use of the main
functions in the package. In this section, we describe the S3
infrastructure underlying our package, including \texttt{print},
\texttt{summary}, and \texttt{plot} methods for the output of the main
functions.

\subsection{model\_importance\_tbl
class}\label{model_importance_tbl-class}
