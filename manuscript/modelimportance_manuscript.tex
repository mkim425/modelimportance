...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, borderline west={3pt}{0pt}{shadecolor}, sharp corners, interior hidden, breakable, enhanced, frame hidden]}{\end{tcolorbox}}\fi
\makeatother
...
(\citet{Rpackage-hubExamples}). The \pkg{modelimportance} uses a
...
models relies on the \pkg{hubEnsembles}, which offers a broadly
applicable framework to construct multi-model ensembles using various
ensemble methods. Calculation of forecast accuracy using various metrics
is based on the \pkg{hubEvals}, which internally leverage the
\pkg{scoringutils}. We use or modify the example datasets from the
\pkg{hubExamples} for testing and demonstration purposes (see Section
\ref{sec:examples}).
...
17, 2022, in the \texttt{model\_out\_tbl} format. The \texttt{model\_id}
...
\texttt{location}, \texttt{target\_date}), the \texttt{output\_type}
column specifying the output type of the predictions and an
\texttt{oracle\_value} column for the observed values. As in the
forecast data, if the \texttt{output\_type} is either
...
the \pkg{hubEnsembles} package. The currently supported methods are
\texttt{"simple\_ensemble"} and \texttt{"linear\_pool"}. The
...
\citep{Rpackage-testthat} to ensure the correctness of all functions,
including those used internally. Additionally, we performed continuous
integration testing using GitHub Actions to maintain code quality and
functionality across platforms, including Windows, macOS, and Linux.
...
Figure~\ref{fig-example-mean-lomo} illustrates these point forecasts and
the eventually observed values for each location.
...
training is infeasible as the data available for training is limited.
