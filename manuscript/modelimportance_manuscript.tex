...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, frame hidden, interior hidden, breakable, boxrule=0pt, enhanced, borderline west={3pt}{0pt}{shadecolor}]}{\end{tcolorbox}}\fi
\makeatother
\usepackage{bookmark}
...
to quantify how each component model contributes to the accuracy of
...
understand the extent to which individual features' contribute to the
model's predictive power (\citet{lundberg2017unified};
\citet{lundberg2020local}; \citet{covert2020understanding}). Related
work by \citet{lipiecki2024postprocessing} used Shapley-based
...
\pkg{PostForecasts.jl} \citep{Jlpackage-lipiecki2025postforecast}.
\pkg{PostForecasts.jl} is conceptually aligned with our work in that
both approaches leverage Shapley values to evaluate component
forecasters. However, while \pkg{PostForecasts.jl} was developed for
\proglang{Julia} users originally focusing on energy economics, our
\proglang{R} package focuses on epidemiological forecasts and offers a
broader range of capabilities, such as handling missing forecasts.
...
and then close this paper with concluding remarks and a discussion of
possible extensions.
...
model was apparently overconfident. \#\#\# Forecast data representation
\{\#subsec:model\_output\}
...
have different weights during the aggregating process.
...
\texttt{model\_importance()} function returns an object of S3 class
...
including key statistics such as number of models and tasks evaluated.
It also displays top scoring models for a subset of tasks. Additional
summary details are available by specifying individual elements of the
summary object as follows:
...
\item
  ...
  tasks. This information allows users to quickly identify the most
...
This function returns a data frame with columns \texttt{model\_id} and
...
minimum model requirement, alongside with a note on how to use parallel
...
As indicated in the output, more details of the summary are accessed
through the elements of the summary object as below.
...
in the \texttt{top\_model} column with its importance score in the
...
model importance. As we explored the difference of \texttt{na\_action}
...
computational experiment with simulated date. The execution time was
...
All experiments were performed on a machine running macOS 15 with an
Apple M4 chip and 16 GB RAM under R version 4.4.3. Parallel computations
were implemented using the \pkg{future} (\texttt{multisession} backend)
with 4 worker processes. The execution times were recorded in seconds.
...
range 0 to 12 seconds for LOMO and 0 to 1110 seconds for LASOMO.
Overall, the time needed to compute model importance scores increases
with the number of models and tasks for both algorithms. Specifically,
when the number of tasks is 10 or 20, the difference in computation time
between sequential and parallel executions is not significant. However,
as the number of prediction tasks rises to 50, the difference becomes
more noticeable. The efficiency of parallel execution is more pronounced
for LASOMO than for LOMO. In particular, for a high volume of prediction
tasks, it results in a substantial improvement when 8 or more models are
involved in the ensemble construction.
...
computational challenges in practice, particularly with large number of
models.
