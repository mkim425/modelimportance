...
`hubEnsembles` (Shandross et al. (2025)), `hubEvals` (Reich et al.
(2025)), and `hubExamples` (Ray et al. (2025)). `modelimportance` uses a
`model_out_tbl` S3 class as the model output format defined in
`hubUtils`, which consists of utility functions to standardize
...
on `hubEvals`, which internally leverages `scoringutils`. We use the
example datasets from `hubExamples` for testing and demonstration
purposes (see [Section 6](#sec:examples)).
...
the `hubExamples` package. {.table .table .table-striped .table-hover
...
target data is sourced from the ‘hubExamples’ package, which provides
sample datasets for multiple modeling hubs in the hubverse format.
