...
Optional progress bars are displayed via the `progressr` package when it
...
model importance scores calculated for individual prediction tasks can
be aggregated across multiple tasks to obtain an overall importance
score for each model via the
[`aggregate()`](https://rdrr.io/r/stats/aggregate.html) method for
`model_imp_tbl` objects. Users can summarize the task-level scores with
specified summary statistics of interest (e.g., mean, median, quantiles)
for a more comprehensive view of model importance. See
[`aggregate.model_imp_tbl()`](https://mkim425.github.io/modelimportance/reference/aggregate.model_imp_tbl.md)
